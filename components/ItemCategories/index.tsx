    ...
    return (
        <div className=' w-full h-full flex items-center bg-[#D2301C] justify-center px-6 ' >
            {CategoryTab.map((item: string, index: number) => {
                return( 
                    <button onClick={()=> clickHandler(item)} key={index} className={item !== tab ? ' w-full flex items-center justify-center bg-[#D2301C] text-white h-[50px] ': ' w-full flex items-center justify-center h-[50px] bg-white text-[#0dadf7]  '} >
