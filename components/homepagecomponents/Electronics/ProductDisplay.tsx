...
export default function ProductDisplay({ item, index }: { item: { image: string, price: number, discount: number, sold: number, _id: string, rate: number, itemName: string, name: string }, index: number }) {
    ...
    return (
        <Box pos="relative" mr="20px" w={["150px", "150px", "150px", "275px"]}>
            <Center
                onClick={() => {
                    IconProduct()
                }}
                zIndex={20} pos="absolute" bg="#fff" w="40px" h="40px" pt="10px" overflow="hidden" borderRadius="50px" right={["24px", "24px", "24px", "50px"]} top={["12px", "12px", "12px", "25px"]}>
                <Rating
            ...
            <Box w={"full"} _hover={{ transform: "scale(1.03)", transition: "0.5s ease-in-out" }} cursor="pointer" p="10px" pt={["0px","0px","0px","10px"]} pb={["0px","0px","0px","10px"]} bg="#ffffff" borderRadius="24px" h={["250px", "250px", "250px", "320px"]} onClick={() => navigateToProductDetails(item._id)} key={index}>
                <Center borderRadius={"20px"} h={["150px", "150px", "150px", "200px"]} overflow={"hidden"}>
